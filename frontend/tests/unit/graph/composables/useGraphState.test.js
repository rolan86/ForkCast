...
describe('useGraphState', () => {
  ...
  describe('initialization', () => {
    it('should return reactive state object', () => {
      ...
      expect(composable.graphState.value).toBeDefined()
      expect(composable.graphState.value.layout).toBe(LAYOUT_TYPES.FORCE)
      expect(composable.graphState.value.visualMode).toBe(VISUAL_MODES.TWO_POINT_FIVE_D)
      expect(composable.graphState.value.renderMode).toBe(RENDER_MODES.HYBRID)
    })

    it('should initialize with default clustering state', () => {
      expect(composable.graphState.value.clustering.enabled).toBe(false)
      expect(composable.graphState.value.clustering.autoDetect).toBe(true)
      expect(composable.graphState.value.clustering.expandedClusters).toBeInstanceOf(Set)
    })

    it('should initialize with default selection state', () => {
      expect(composable.graphState.value.selection.nodes).toEqual([])
      expect(composable.graphState.value.selection.mode).toBe(INTERACTION_MODES.SELECT)
      expect(composable.graphState.value.selection.pathStart).toBeNull()
      expect(composable.graphState.value.selection.pathEnd).toBeNull()
      expect(composable.graphState.value.selection.neighborHops).toBe(1)
    })

    it('should initialize with default view state', () => {
      expect(composable.graphState.value.view.zoom).toBe(1)
      expect(composable.graphState.value.view.pan).toEqual({ x: 0, y: 0 })
      expect(composable.graphState.value.view.viewport).toEqual({ x: 0, y: 0, w: 0, h: 0 })
    })

    it('should initialize with performance mode enabled', () => {
      expect(composable.graphState.value.performance.animationsEnabled).toBe(true)
      expect(composable.graphState.value.performance.performanceMode).toBe(false)
    })
  })
  ...
  describe('updateLayout', () => {
    it('should update layout state', () => {
      ...
      expect(composable.graphState.value.layout).toBe(LAYOUT_TYPES.HIERARCHICAL)
    })
    ...
    it('should not update with invalid layout', () => {
      const originalLayout = composable.graphState.value.layout
      composable.updateLayout('invalid-layout')
      expect(composable.graphState.value.layout).toBe(originalLayout)
    })
  })
  ...
  describe('updateLayoutParam', () => {
    it('should update layout parameter', () => {
      ...
      expect(composable.graphState.value.layoutParams[LAYOUT_TYPES.FORCE].linkDistance).toBe(100)
    })

    it('should not update non-existent layout', () => {
      const originalParams = { ...composable.graphState.value.layoutParams[LAYOUT_TYPES.FORCE] }
      composable.updateLayoutParam('non-existent', 'param', 100)
      expect(composable.graphState.value.layoutParams[LAYOUT_TYPES.FORCE]).toEqual(originalParams)
    })
  })
  ...
  describe('updateVisualMode', () => {
    it('should update visual mode', () => {
      ...
      expect(composable.graphState.value.visualMode).toBe(VISUAL_MODES.TWO_D)
    })

    it('should not update with invalid visual mode', () => {
      const originalMode = composable.graphState.value.visualMode
      composable.updateVisualMode('invalid-mode')
      expect(composable.graphState.value.visualMode).toBe(originalMode)
    })
  })
  ...
  describe('updateRenderMode', () => {
    it('should update render mode', () => {
      ...
      expect(composable.graphState.value.renderMode).toBe(RENDER_MODES.CANVAS)
    })

    it('should not update with invalid render mode', () => {
      const originalMode = composable.graphState.value.renderMode
      composable.updateRenderMode('invalid-mode')
      expect(composable.graphState.value.renderMode).toBe(originalMode)
    })
  })

  describe('autoSelectRenderMode', () => {
    it('should be a no-op stub (deprecated)', () => {
      const originalMode = composable.graphState.value.renderMode
      composable.autoSelectRenderMode(50)
      expect(composable.graphState.value.renderMode).toBe(originalMode)
    })

    it('should not change render mode for any node count', () => {
      const originalMode = composable.graphState.value.renderMode
      composable.autoSelectRenderMode(300)
      expect(composable.graphState.value.renderMode).toBe(originalMode)
    })
  })
  ...
  describe('updateSelection', () => {
    it('should update selection nodes array', () => {
      ...
      expect(composable.graphState.value.selection.nodes).toEqual(['node1', 'node2'])
    })

    it('should convert single node to array', () => {
      composable.updateSelection({ nodes: 'node1' })
      expect(composable.graphState.value.selection.nodes).toEqual(['node1'])
    })

    it('should update selection mode', () => {
      composable.updateSelection({ mode: INTERACTION_MODES.PATH })
      expect(composable.graphState.value.selection.mode).toBe(INTERACTION_MODES.PATH)
    })
    ...
    it('should update path start and end', () => {
      ...
      expect(composable.graphState.value.selection.pathStart).toBe('node1')
      expect(composable.graphState.value.selection.pathEnd).toBe('node2')
    })

    it('should update neighbor hops', () => {
      composable.updateSelection({ neighborHops: 3 })
      expect(composable.graphState.value.selection.neighborHops).toBe(3)
    })
  })
  ...
  describe('addNodeToSelection', () => {
    it('should add node to selection', () => {
      ...
      expect(composable.graphState.value.selection.nodes).toContain('node1')
    })
    ...
    it('should respect max 10 nodes limit', () => {
      ...
      expect(composable.graphState.value.selection.nodes.length).toBe(10)
      expect(composable.graphState.value.selection.nodes).not.toContain('node10')
    })
  })
  ...
  describe('removeNodeFromSelection', () => {
    it('should remove node from selection', () => {
      ...
      expect(composable.graphState.value.selection.nodes).toEqual(['node1', 'node3'])
    })

    it('should handle removing non-existent node', () => {
      const originalNodes = [...composable.graphState.value.selection.nodes]
      composable.removeNodeFromSelection('non-existent')
      expect(composable.graphState.value.selection.nodes).toEqual(originalNodes)
    })
  })
  ...
  describe('updateView', () => {
    it('should update zoom level', () => {
      ...
      expect(composable.graphState.value.view.zoom).toBe(2)
    })

    it('should update pan position', () => {
      composable.updateView({ pan: { x: 100, y: 200 } })
      expect(composable.graphState.value.view.pan).toEqual({ x: 100, y: 200 })
    })
    ...
    it('should update viewport bounds', () => {
      ...
      expect(composable.graphState.value.view.viewport).toEqual({ x: 10, y: 20, w: 400, h: 300 })
    })

    it('should merge partial updates', () => {
      composable.updateView({ zoom: 1.5 })
      expect(composable.graphState.value.view.zoom).toBe(1.5)
      expect(composable.graphState.value.view.pan).toEqual({ x: 0, y: 0 })
    })
  })
  ...
  describe('toggleCluster', () => {
    it('should add cluster to expanded set when collapsed', () => {
      ...
      expect(composable.graphState.value.clustering.expandedClusters.has('cluster1')).toBe(true)
    })

    it('should remove cluster from expanded set when expanded', () => {
      composable.toggleCluster('cluster1')
      composable.toggleCluster('cluster1')
      expect(composable.graphState.value.clustering.expandedClusters.has('cluster1')).toBe(false)
    })
  })
  ...
  describe('updateClustering', () => {
    it('should update clustering enabled state', () => {
      ...
      expect(composable.graphState.value.clustering.enabled).toBe(true)
    })

    it('should update auto-detect state', () => {
      composable.updateClustering({ autoDetect: false })
      expect(composable.graphState.value.clustering.autoDetect).toBe(false)
    })

    it('should merge partial updates', () => {
      composable.updateClustering({ enabled: true })
      expect(composable.graphState.value.clustering.enabled).toBe(true)
      expect(composable.graphState.value.clustering.autoDetect).toBe(true) // unchanged
    })
  })
  ...
  describe('updatePerformanceMode', () => {
    it('should enable performance mode', () => {
      ...
      expect(composable.graphState.value.performance.performanceMode).toBe(true)
      expect(composable.graphState.value.performance.animationsEnabled).toBe(false)
    })
  ...
  describe('updatePathResult', () => {
    it('should update pathResult with array', () => {
      ...
      expect(composable.graphState.value.selection.pathResult).toEqual(['node1', 'node2', 'node3'])
    })

    it('should reset pathResult with empty array for non-array input', () => {
      composable.updatePathResult(null)
      expect(composable.graphState.value.selection.pathResult).toEqual([])
    })
  })
  ...
  describe('updateNeighborResult', () => {
    it('should update neighborResult with array', () => {
      ...
      expect(composable.graphState.value.selection.neighborResult).toEqual(['nodeA', 'nodeB'])
    })
  })
  ...
  describe('updateRenderMode with user selection flag', () => {
    it('should set _userSelectedRenderMode when isUserSelection is true', () => {
      ...
      expect(composable.graphState.value.renderMode).toBe(RENDER_MODES.CANVAS)
      expect(composable.graphState.value._userSelectedRenderMode).toBe(true)
    })

    it('should not set flag when isUserSelection is false', () => {
      composable.updateRenderMode(RENDER_MODES.SVG, false)
      expect(composable.graphState.value._userSelectedRenderMode).toBe(false)
    })
  })

  describe('reactive behavior', () => {
    it('should make graphState reactive', () => {
      const initialState = composable.graphState.value.layout
      composable.updateLayout(LAYOUT_TYPES.CIRCULAR)
      expect(composable.graphState.value.layout).toBe(LAYOUT_TYPES.CIRCULAR)
      expect(composable.graphState.value.layout).not.toBe(initialState)
    })

    it('should maintain immutability of arrays', () => {
      const originalArray = composable.graphState.value.selection.nodes
      composable.updateSelection({ nodes: ['new-node'] })
      expect(composable.graphState.value.selection.nodes).not.toBe(originalArray)
    })
  })
})
